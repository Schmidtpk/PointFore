...
The main function is *estimate.functional()* which creates an object of class *pointfore*. Many paramters are available but most of them can be set to their default values. They are explained in details below through examples.
...
The *logistic* model is already implemented in the *PointFore* package. The level of the forecast depends on the the state variable s, via the formula
...
It is straigt forward to apply standard Wald tests to the estimated *gmm-object*. First, we have to load the car package.
...
Similarly, the flexible logistic model cannot reject the mean-forecast ($H_0: \theta_1=0 \wedge \theta_2 =0$), nor state-independence($H_0: \theta_2 =0$).
...
The two parameters stateVariable and theta are necessary. The output is the level and should always be in the unit interval.
...
A bias that can also be detected by the logistic model, which has a similar shape.

```{r}
res.logistic <- estimate.functional(iden.fct = expectiles,
                                    model = logistic,
                                    Y = Y,X = X,
                                    theta0 = c(0,0),
                                    stateVariable = X)
...
However, the biased forecast is **not optimal with respect to the logistic model**. The deviation can be detected by the J-test of overidentifying restrictions if we use appropriate instruments, that generate a sufficiently rich information set to test for.

If we include the squared forecast with its original sign in the instruments, the test rejects optimality with respect to the logistic model with a p-value below 0.01 and correctly cannot reject optimality with respect to the break model with a p-value of 0.09.
